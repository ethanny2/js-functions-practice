/* subStrCheck
  1) Returns number of instances of substring "abba" in input string. 
  It is case insensitive so "abba" counts, and so does "aBbA".
  We also want to check for overlapping abba's such as in the String
  "abbAbba" that contains "abba" twice.
  subStrCheck("AbBa a b b a") returns 1
  subStrCheck("abbabba!") returns 2
*/
function subStrCheck(str) {
  var counter = 0;
  const substr = "abba";
  const offSet = 2;
  var position = 0;
  while (str.toLowerCase().indexOf(substr, position) != -1) {
    position = str.toLowerCase().indexOf(substr, position) + offSet;
    counter++;
  }
  return counter;
}

console.assert(0 == subStrCheck("no"));
console.assert(1 == subStrCheck("axxc AbBa Xx"));
console.assert(2 == subStrCheck("abbabba"));
console.assert(2 == subStrCheck("abbabba!"));
console.assert(1 == subStrCheck("AbBa a b b a"));
console.assert(4 == subStrCheck("abBABBABBAF2434ABBa"));
...
function isStringSorted(str) {
  ...
  while (pos < str.length) {
    if (str.charAt(pos).charCodeAt() > str.charAt(pos + 1).charCodeAt()) {
      return false;
    }
    pos++;
  }
...
/*
  ...
  of 1s in the array, continuing until the 11th value (at index 10) is the number of tens in the array.
...
console.assert(1 == arr[0]);
console.assert(0 == arr[1]);
console.assert(2 == arr[3]);
console.assert(4 == arr[8]);
console.assert(0 == arr[9]);
console.assert(2 == arr[10]);
